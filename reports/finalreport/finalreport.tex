...
Interestingly enough, the ColorUNet sometimes gives results that are not similar to the input image, but look better. Those examples are interesting, because they reveal how the coloring model works. Some of them are reported in figure \ref{better}

\begin{figure*}
\begin{center}
\includegraphics[width=450px]{good}
\caption{Sample predictions of the ColorUNet on the validation set, for several prediction temperatures. The temperature parameter allows a tradeoff between vivid colors and elimation of artifacts of a wrong color}
\label{good}
\end{center}
\end{figure*}
...
We also have cases where the colorizer gives wrong results, because of very specific input images. For example, a snowy ground is guessed to be grass by the classifier, which outputs very interesting results. 

We can also see that in some casesm, our colorizer is completely wrong, and we can quite easily understand why. 


To better understand the bahavior of our model, we can have a look at a \textbf{certainty} map, as in figure \ref{•}.
