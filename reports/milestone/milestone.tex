...
Using this trained model to colorize an out-of-sample picture, we see that it has captured the notions of beach and sky (coloring the former in yellow tones and the latter in blue). As we notice (Figure \ref{oos}), the model still has trouble handling the clouds and separating the sea from the rest, but this result is definitely encouraging.
