...
For now, given that we only work on the test and validation set, we will only consider the classification loss. As a result, our current focus is to find a network that is efficient at solving the classification task that we used to "encode" the colorization problem. The loss of our network is our main metric (it will be describes more precisely in section ???). As we are writing this milestone, we want, as a sanity check, to confirm that we are able to overfit the dataset with a quite complex model. To check this, we simply tune the parameters so that the network has a consistent behavior in terms of loss and actually colorizes images from the training set.
